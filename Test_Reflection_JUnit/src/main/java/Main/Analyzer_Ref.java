...
public class Analyzer_Ref {
    ...
    private static String generateTestMethod(Class<?> clazz, Method method, int testCaseCount) {
        ...
        for (int i = 0; i < testCaseCount; i++) {
            ...
            String[] csvValues = new String[params.length + 1]; // +1 for expected result
            for (int j = 0; j < params.length; j++) {
                csvValues[j] = csvFormatValue(inputs[j]);
            }
            csvValues[params.length] = (expected != null) ? csvFormatValue(expected) : "null";
    ...
    private static String csvFormatValue(Object value) {
        if (value == null) {
            return "null";
        }
    ...
    public static void writeTestFile(Class<?> clazz, int testCaseCount) {
        try {
            String testCode = TestCases(clazz, testCaseCount);
